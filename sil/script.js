...
(async function() {
  ...
  // github-upload.js
// Client-side uploader to GitHub repo using Personal Access Token (PAT).
// - Token is stored in sessionStorage only.
// - Call githubUploadIfConfigured() to attempt upload (it reads family from localStorage).
// - Intended for admin use only (do not expose PAT to public).

(function() {
  // sessionStorage keys
  const KEY_TOKEN = 'gh_pat_token';
  const KEY_OWNER = 'gh_repo_owner';
  const KEY_REPO = 'gh_repo_name';
  const KEY_BRANCH = 'gh_repo_branch';
  const KEY_PATH = 'gh_repo_path';
  ...
  window.showGithubModal = function() {
    ...
    $('ghOwner').value = sessionStorage.getItem(KEY_OWNER) || '';
    $('ghRepo').value = sessionStorage.getItem(KEY_REPO) || '';
    $('ghBranch').value = sessionStorage.getItem(KEY_BRANCH) || '';
    $('ghPath').value = sessionStorage.getItem(KEY_PATH) || 'data/silsilah.json';
    $('ghToken').value = sessionStorage.getItem(KEY_TOKEN) || '';
  ...
  window.closeGithubModal = function() {
    ...
    document.body.style.overflow = '';
    // clear token input value for safety
    $('ghToken').value = '';
  };

  // save config (token stored in sessionStorage)
  window.saveGithubConfig = function() {
    const owner = $('ghOwner').value.trim();
    const repo = $('ghRepo').value.trim();
    const branch = $('ghBranch').value.trim() || 'gh-pages';
    const path = $('ghPath').value.trim() || 'data/silsilah.json';
    const token = $('ghToken').value.trim();

    if (!owner || !repo || !path) {
      alert('Owner, repo, dan path harus diisi.');
      return;
    }
    // token optional (you can set it later), but if set we store in sessionStorage
    ...
    sessionStorage.setItem(KEY_BRANCH, branch);
    sessionStorage.setItem(KEY_PATH, path);

    closeGithubModal();
    alert('Pengaturan GitHub disimpan di session. Token hanya disimpan di session.');
  };

  // clear config (remove token)
  window.clearGithubConfig = function() {
    if (!confirm('Hapus konfigurasi GitHub (termasuk token yang tersimpan di session)?')) return;
    sessionStorage.removeItem(KEY_TOKEN);
    sessionStorage.removeItem(KEY_OWNER);
    sessionStorage.removeItem(KEY_REPO);
    sessionStorage.removeItem(KEY_BRANCH);
    sessionStorage.removeItem(KEY_PATH);
    closeGithubModal();
    alert('Konfigurasi GitHub dihapus dari session.');
  };

  // read config (return null if incomplete)
  function getGithubConfig() {
    const owner = sessionStorage.getItem(KEY_OWNER);
    const repo = sessionStorage.getItem(KEY_REPO);
    const branch = sessionStorage.getItem(KEY_BRANCH) || 'gh-pages';
    const path = sessionStorage.getItem(KEY_PATH) || 'data/silsilah.json';
    const token = sessionStorage.getItem(KEY_TOKEN) || null;
  ...
  // uploadToGitHub: create/update file via Contents API
  async function uploadToGitHub({ owner, repo, path, branch = 'gh-pages' }, token, contentStr, commitMessage) {
    ...
    try {
      const resGet = await fetch(`${fileUrl}?ref=${encodeURIComponent(branch)}`, {
      ...
      if (resGet.ok) {
        ...
        // non-404 error — throw for caller to handle
        const txt = await resGet.text();
        throw new Error(`GET contents failed: ${resGet.status} ${txt}`);
      }
    } catch (err) {
      // If network error, propagate
      console.warn('GET contents error (continuing to attempt create):', err);
    }

    const contentBase64 = utf8ToBase64(contentStr || '');
    const body = {
      message: commitMessage || (existingSha ? `Update silsilah ${new Date().toISOString()}` : `Create silsilah ${new Date().toISOString()}`),
    ...
    if (!res.ok) {
      ...
      throw new Error(`Upload to GitHub failed: ${res.status} ${txt}`);
    }
    return await res.json();
  }

  // Attempt upload using current config (if token present). Returns promise.
  window.githubUploadIfConfigured = async function() {
    const cfg = getGithubConfig();
    if (!cfg) {
      // no config: nothing to do
      console.debug('[github-upload] no config found in sessionStorage');
      return null;
    }
    if (!cfg.token) {
      console.warn('[github-upload] token not found in sessionStorage - open modal to set token');
      return null;
    }

    // read family JSON from localStorage
    let family = null;
    try {
      family = JSON.parse(localStorage.getItem('silsilah_family'));
    } catch (e) { family = null; }
    if (!family) {
      console.warn('[github-upload] no family data found in localStorage to upload');
      return null;
    }

    const contentStr = JSON.stringify(family, null, 2);
    try {
      const result = await uploadToGitHub({ owner: cfg.owner, repo: cfg.repo, path: cfg.path, branch: cfg.branch }, cfg.token, contentStr);
      console.info('[github-upload] upload success', result);
      // update UI: we set lastFetchedJSON so updateSyncUI can show a timestamp (if your script uses lastFetchedJSON)
      try { window.lastFetchedJSON = JSON.stringify(family); } catch (e) {}
      // optionally update sync UI if exposed
      if (typeof window.setSyncStatus === 'function') {
        window.setSyncStatus('idle', 'Tersinkron ke GitHub');
      }
      return result;
    } catch (err) {
      console.error('[github-upload] upload failed', err);
      if (typeof window.setSyncStatus === 'function') {
        window.setSyncStatus('failed', 'Gagal sinkron GitHub: ' + (err.message || 'error'));
      }
      throw err;
    }
  };

  // Hook into global markDirtyAndScheduleUpload by observing if function exists:
  // If your existing script defines markDirtyAndScheduleUpload (as in previous versions),
  // we patch it to trigger githubUploadIfConfigured after it schedules the upload.
  function tryPatchMarkDirty() {
    if (!window.markDirtyAndScheduleUpload) return;
    // wrap original
    const orig = window.markDirtyAndScheduleUpload;
    window.markDirtyAndScheduleUpload = function(...args) {
      // call original
      const res = orig.apply(this, args);
      // schedule GitHub upload (debounced slightly) if config present
      const cfg = getGithubConfig();
      if (cfg && cfg.token) {
        // small delay to ensure data saved to localStorage by original
        setTimeout(() => {
          window.githubUploadIfConfigured().catch(e => {
            console.warn('github auto-upload failed', e);
          });
        }, 1200);
      }
      return res;
    };
    console.debug('[github-upload] patched markDirtyAndScheduleUpload to trigger github upload if configured');
  }

  // attempt to patch on load and also when DOM ready
  tryPatchMarkDirty();
  document.addEventListener('DOMContentLoaded', tryPatchMarkDirty);

  // attach close handler for modal close button if present
  document.addEventListener('click', function(e) {
    if (e.target && e.target.id === 'githubClose') {
      closeGithubModal();
    }
  });

  // expose a function to check if GitHub config exists
